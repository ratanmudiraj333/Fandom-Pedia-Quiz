...
function AnimeDisplay()
{
    //const [show, setShow] = useState(AData.map(CreateAnime));
    const [show1, setShow1] = useState(AData2.map(CreateAnime));
    const [show2, setShow2] = useState(AData3.map(CreateAnime));
function display1()
{
    setShow1(AData1.map(CreateAnime));
    setShow2("");
/*<div>
{show && <h1>Anime</h1>}
</div>*/
...
function display2()
{
    setShow1(AData2.map(CreateAnime));
    setShow2("");
}

function display3()
{
    setShow1(AData3.map(CreateAnime));
    setShow2("");
}

function display4()
{
    setShow1(AData4.map(CreateAnime));
    setShow2("");
}

function display5()
{
    setShow1(AData5.map(CreateAnime));
    setShow2("");
}

function display6()
{
    setShow1(AData6.map(CreateAnime));
    setShow2("");
}
...
{
    setShow1(AData2.map(CreateAnime));
    setShow2(AData3.map(CreateAnime));
}
...
<div className="btn-group-vertical" role="group" aria-label="Vertical button group">
  <button type="button" className="btn btn-primary bta" onClick = {displayall}>All characters</button>
...
<input className = "search" type = "text" placeholder="Enter character name" onChange={(e)=>setSearch(e.target.value)} onClick={blank}/>
